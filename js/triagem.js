/**
 * js/triagem.js
 * Versão v77.4 - Módulo de Triagem Inteligente com Validação Oficial CTT, Bricks Dinâmicos e Cloud Sync
 * Faz: Controla toda a lógica de triagem, cálculo de motorista e Brick designados para o código postal de 7 dígitos,
 *      integra a base oficial de ruas do CTT (CP7_DATABASE), suporta Bricks Dinâmicos, OCR com câmara e estatísticas do turno.
 * Depende de: ./geografia-data.js, ./cp7-data.js, ./storage.js, ./ui.js, ./firebase-init.js
 */
...
export function obterConcelhoPorCodigoPostal(zip) {
    ...
    // Consulta primeiro a verdade absoluta dos CTT
    if (CP7_DATABASE && CP7_DATABASE[cleanZip] && CP7_DATABASE[cleanZip].concelho) {
        return CP7_DATABASE[cleanZip].concelho.toUpperCase();
    }

    const cleanPrefix = sanitizeDigits(cleanZip).substring(0, 4);
    // Códigos postais de Sintra começam tipicamente por 27xx
    if (cleanPrefix.startsWith("27")) {
        return "SINTRA";
    }
    return "MAFRA";
}
...
export function findBrickAndDriverForZip(zip, drivers) {
    if (!zip || !drivers) return { brickId: null, brickName: null, driver: null, ruaOficial: null };
    const normalizedZip = zip.trim();
    
    const concelho = obterConcelhoPorCodigoPostal(normalizedZip);
    const dadosCtt = (CP7_DATABASE && CP7_DATABASE[normalizedZip]) ? CP7_DATABASE[normalizedZip] : null;
    const ruaOficial = dadosCtt ? (dadosCtt.rua || "") : "";

    // 1. PRIORIDADE MÁXIMA: Verifica se o CP7 pertence a um Brick Dinâmico criado pelo utilizador
    const customBricks = safeJSONParse('cp_custom_bricks', []);
    const dynamicBrick = customBricks.find(b => Array.isArray(b.cp7List) && b.cp7List.includes(normalizedZip));

    if (dynamicBrick) {
        const designatedDriver = drivers.find(d => d.id === dynamicBrick.driverId);
        return {
            brickId: dynamicBrick.id,
            brickName: dynamicBrick.nome,
            driver: designatedDriver || null,
            concelho: (dynamicBrick.concelho || concelho).toUpperCase(),
            ruaOficial: ruaOficial,
            localidadeOficial: dadosCtt ? (dadosCtt.localidade || dadosCtt.cpalf) : "",
            isDynamic: true
        };
    }

    // 2. SEGUNDA PRIORIDADE: Resolução Geográfica Estática por Freguesia
    let matchedFreguesia = null;
    let matchedLocalidade = null;

    if (GEOGRAPHY && GEOGRAPHY[concelho]) {
        // PASSAGEM 1: Mira laser - Procura apenas nas localidades específicas (ignorando catch-all)
        for (const [freguesia, localidades] of Object.entries(GEOGRAPHY[concelho])) {
            for (const [localidade, cpList] of Object.entries(localidades)) {
                ...
                if (Array.isArray(cpList) && cpList.includes(normalizedZip)) {
    ...
    // 3. TERCEIRA PRIORIDADE: Se encontrou na base oficial CTT mas não nos ficheiros legados
    if (!matchedFreguesia && dadosCtt) {
        matchedFreguesia = dadosCtt.cpalf || dadosCtt.concelho;
        matchedLocalidade = dadosCtt.localidade || dadosCtt.rua || "Zona Postal";
    }

    if (!matchedFreguesia) {
        return { brickId: null, brickName: null, driver: null, concelho, ruaOficial };
    }
...
export function renderSummary(assignments, drivers, painelResumo) {
    ...
    const totalLeituras = assignments ? assignments.length : 0;
    const totalPrioritarios = assignments ? assignments.filter(a => a.priority === true).length : 0; 
...
// =========================================================================
// LÓGICA DE DETEÇÃO DE CÓDIGOS E MODAL DE LEITURAS (TRIAGEM OFICIAL)
// =========================================================================
export function setupTriagemLogic() {
    ...
    if (btnAnalisar) {
        btnAnalisar.addEventListener('click', () => {
            ...
            // Resolução inteligente priorizada
            const { brickId, brickName, driver, concelho, ruaOficial, localidadeOficial } = findBrickAndDriverForZip(formattedZip, window.drivers);
            ...
            if (!brickId) {
                ...
                if (resultadoBrickLabel) {
                    resultadoBrickLabel.textContent = ruaOficial ? `${ruaOficial} (${concelho})` : `Confirmar Código Postal (${concelho})`;
                }
                if (resultadoCorBg) resultadoCorBg.style.backgroundColor = "#EA580C";
                ...
                window.lastAnalysisResult = { 
                    ...
                    concelho: concelho,
                    ruaOficial: ruaOficial,
                    localidadeOficial: localidadeOficial
                };
            } else {
                if (resultadoBrickLabel) {
                    const descRua = ruaOficial ? `${ruaOficial} | ` : '';
                    resultadoBrickLabel.textContent = `${descRua}${brickName} (${concelho})`;
                }
    ...
    if (btnConfirmarAtribuir && modalResultado) {
        btnConfirmarAtribuir.addEventListener('click', () => {
            ...
            // Grava a leitura de triagem enriquecida com a morada CTT no Firestore
            db.collection('assignments').add({
                id: 'a_' + Date.now(),
                zip: window.lastAnalysisResult.zip,
                rua: window.lastAnalysisResult.ruaOficial || "",
                localidade: window.lastAnalysisResult.localidadeOficial || "",
...
// ==========================================
// CONFIGURAÇÃO DOS BOTÕES "CANCELAR"
// ==========================================
export function setupCancelButtons() {
    ...
    if (btnCancelarMotorista) {
        btnCancelarMotorista.addEventListener('click', () => {
            if (window.cancelarEdicaoDriver) window.cancelarEdicaoDriver();
        });
    }

    if (btnCancelarSetor) {
        btnCancelarSetor.addEventListener('click', () => {
            if (window.cancelarEdicaoSector) window.cancelarEdicaoSector();
        });
    }
}
...
// ==========================================
// CONFIGURAÇÃO DOS EVENTOS DA CÂMARA OCR COM VALIDAÇÃO CTT
// ==========================================
    ...
    inputCamera.addEventListener('change', (event) => {
        ...
        preprocessarImagemParaOCR(file, (processedBlob) => {
            Tesseract.recognize(
                ...
                // 1. PRIORIDADE MÁXIMA: Tenta encontrar diretamente um CP7 no texto lido
                const regexCp7 = /\b(\d{4})[- ]?(\d{3})\b/;
                const matchCp = text.match(regexCp7);

                if (matchCp) {
                    const extractedCp = `${matchCp[1]}-${matchCp[2]}`;
                    if (CP7_DATABASE && CP7_DATABASE[extractedCp]) {
                        window.currentInput = sanitizeDigits(extractedCp);
                        const visorCodigo = document.getElementById('visor-codigo');
                        if (visorCodigo) {
                            updateVisor(window.isPrefixLocked, window.lockedPrefixValue, window.currentInput, visorCodigo);
                        }
                ...
                // 2. SEGUNDA PRIORIDADE: Fallback via Geocoding Google Maps
